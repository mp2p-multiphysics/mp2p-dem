...
void CollisionCheckSphereSphereSweepPrune::set_input_parameter(VectorDouble radius_vec_in, double enlarge_factor_in = 1.05)
{
    radius_vec = radius_vec_in;
    enlarge_factor = enlarge_factor_in;
}
...
VectorPairInt CollisionCheckSphereSphereSweepPrune::broad_search(SpherePositionVelocityStruct &sphere_pvs)
{
    ...
    for (int indx_i = 0; indx_i < sphere_pvs.num_particle; indx_i++)
    {
        ...
        double rad_i = enlarge_factor*radius_vec[type_i];
    ...
    for (auto &pair_sub : indx_checkpair_x_vec)
    {
        ...
        double rad_i = enlarge_factor*radius_vec[type_i];
        double rad_j = enlarge_factor*radius_vec[type_j];

        // calculate lower and uppper bounds
        double lower_bound_y_i = pos_y_i - rad_i;
        double lower_bound_y_j = pos_y_j - rad_j;
        double upper_bound_y_i = pos_y_i + rad_i;
        double upper_bound_y_j = pos_y_j + rad_j;
    ...
    for (auto &pair_sub : indx_checkpair_xy_vec)
    {
        ...
        // calculate lower and uppper bounds
        double lower_bound_z_i = pos_z_i - rad_i;
        double lower_bound_z_j = pos_z_j - rad_j;
        double upper_bound_z_i = pos_z_i + rad_i;
        double upper_bound_z_j = pos_z_j + rad_j;
